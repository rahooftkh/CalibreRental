        ...
        return {
            ...
            link: function link(scope, element, attrs, ctrl) {
                ...
                function startWatch() {
                    ...
                    if (!watcher) {
                        watcher = scope.$watch('currentProperty.value', function (newValue, oldValue) {
                            ...
                            if (errCount === 0 || errCount === 1 && Utilities.isArray(formCtrl.$error.valPropertyMsg) || formCtrl.$invalid && Utilities.isArray(formCtrl.$error.valServer)) {
                                scope.errorMsg = '';
                ...
                if (scope.currentProperty) {
                    ...
                    unsubscribe.push(serverValidationManager.subscribe(scope.currentProperty.alias, currentCulture, '', serverValidationManagerCallback, currentSegment));
                }
    ...
    function valServer(serverValidationManager) {
        return {
            ...
            link: function link(scope, element, attr, ctrls) {
                ...
                unsubscribe.push(serverValidationManager.subscribe(currentProperty.alias, currentCulture, fieldName, serverValidationManagerCallback, currentSegment));
                scope.$on('$destroy', function () {
                    stopWatch();
                    for (var u in unsubscribe) {
                        unsubscribe[u]();
